...
[![Версия](https://img.shields.io/badge/vpn--node--tuner-1.2.1-blue.svg)](#-что-нового)
...
[![Транспорты](https://img.shields.io/badge/TCP_%7C_XHTTP_%7C_gRPC_%7C_WS-supported-orange.svg)](#-транспорты-xray)
...
**[Установка](#-установка)** · **[Команды](#-команды)** · **[Профили](#-профили-по-ram)** · **[Транспорты](#-транспорты-xray)** · **[Что меняется](#-что-именно-меняется-и-зачем)** · **[A/B](#-ab-эксперименты)** · **[Откат](#-откат)** · **[English](./README.md)**

</div>

Интерактивный скрипт, который приводит `/etc/sysctl.conf` в порядок под **VPN-ноду** (Xray / sing-box / 3X-UI / Remnawave) с **TCP-транспортом** — TCP/RAW (в том числе VLESS + Reality + Vision), XHTTP, gRPC, WebSocket, с Mux и без: включает **BBR + fq**, подбирает буферы и очереди **под объём RAM сервера**, снимает исчерпание портов и убирает «раскачку» после паузы в плеере. Плюс всё, без чего тюнинг обычно разваливается: swap, лимиты файловых дескрипторов, бэкапы, откат и диагностика.
...
| Версия | Главное |
|---|---|
| **1.2.1** | «0 — Назад» в выборе профиля и в истории тестов возвращает сразу, без лишнего Enter; неверный номер в подменю тоже больше не требует Enter |
...
Скрипт настраивает TCP-стек сервера, поэтому работает с **любым TCP-транспортом** Xray — включая самый распространённый, **TCP/RAW (VLESS + Reality + Vision)**. Xray не пересылает пакеты как туннель: он сам принимает TCP-соединение клиента и сам открывает TCP-соединение к сайту, и оба соединения обслуживает ядро вашего сервера.

```
клиент ──TCP──▶ Xray на сервере ──TCP──▶ сайт
   BBR, буферы, очереди          порты, tw_reuse, BBR
```

| Транспорт | Соединений клиент → сервер | Что даёт основной эффект |
|---|---|---|
| **TCP / RAW без Mux** (в том числе Reality + Vision) | Много: отдельное на каждое соединение приложения | `bbr` + `fq`, очереди `somaxconn` / `syn_backlog`, лимиты дескрипторов (пункт 6) |
| **WebSocket без Mux** | Много, как у TCP / RAW | То же, что у TCP / RAW |
| **TCP / RAW с Mux**, **XHTTP**, **gRPC**, **HTTP/2** | Одно или несколько на устройство, потоки идут внутри | `bbr` + `fq`, буферы и `tcp_slow_start_after_idle = 0`: одно соединение везёт весь трафик и часами висит с паузами |
| **UDP**: Hysteria2, TUIC, mKCP, QUIC | — | Параметры `tcp_*` не действуют, влияют только `rmem_max` / `wmem_max` |

- Соединения **от Xray к сайтам** всегда отдельные на каждый поток — Mux распаковывается на сервере. Поэтому порты, `tw_reuse` и `fin_timeout` нужны при любом транспорте.
- BBR ускоряет то, что сервер **отправляет**, то есть скачивание на клиент. Скорость выгрузки с клиента на сервер определяет алгоритм на самом клиенте.

**Как узнать, что у вас.** На сервере, пока с одного устройства через VPN открыты несколько сайтов (замените `443` на порт своего inbound):

```bash
ss -tn state established '( sport = :443 )' | tail -n +2 | wc -l
```

Десятки соединений — Mux нет. Одно-два на устройство — Mux или мультиплексирующий транспорт.
...
|---|---|---|
| `net.ipv4.tcp_slow_start_after_idle` | `0` | Не сбрасывает окно перегрузки после паузы. Соединение, которое простояло без трафика (плеер на паузе, открытая вкладка), продолжает на прежней скорости, а не разгоняется с нуля — это убирает «раскачку» при старте Reels/TikTok после паузы. Сильнее всего заметно на Mux, gRPC и XHTTP, где одно соединение висит часами |
