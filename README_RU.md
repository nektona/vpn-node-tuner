...
[![Версия](https://img.shields.io/badge/vpn--node--tuner-1.1.0-blue.svg)](#-что-нового)
...
Пункт меню **4 · 🧪 Тестовый режим**. Значения применяются на лету через `sysctl -w`, **в файл ничего не пишется** и всё сбрасывается при перезагрузке — это позволяет проверить спорный параметр без риска.
